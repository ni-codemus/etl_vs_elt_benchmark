...
resource "aws_instance" "app" {
  ...
  user_data = templatefile("${path.module}/../../bootstrap.sh.tpl", {
    ...
    bench_project_name  = var.project_name
    bench_environment   = var.environment
    bench_ec2_type      = var.ec2_instance_type
    bench_db_instance   = var.db_instance_class
    bench_s3_bucket     = var.s3_results_bucket
    bench_s3_key_prefix = var.s3_results_key_prefix
  })
